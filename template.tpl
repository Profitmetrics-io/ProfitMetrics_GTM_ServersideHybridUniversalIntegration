...
// Set email input Selector and remove a trailing comma if present

if (pmEmailInputSelector) {
  log(pmEmailInputSelector);
  pmEmailInputSelector = pmEmailInputSelector.trim();
  if (pmEmailInputSelector.slice(-1) === ',') {
    pmEmailInputSelector = pmEmailInputSelector.slice(0, -1).trim();
  }
  log(pmEmailInputSelector);
